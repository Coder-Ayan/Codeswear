...
const handler = async (req, res) => {
    if (req.method === 'POST') {
        ...
        if (user) {
            let bytes = CryptoJS.AES.decrypt(user.password, process.env.NEXT_PUBLIC_PASSWORD_SECRET);
            let decryptedPassword = bytes.toString(CryptoJS.enc.Utf8);
            if (req.body.email == user.email && req.body.password == decryptedPassword) {
                let token = jwt.sign({ name: user.name, email: user.email }, process.env.NEXT_PUBLIC_JWT_SECRET, { expiresIn: '7d' });
